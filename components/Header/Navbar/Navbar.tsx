...
export default function Navbar() {
  return (
    <nav className="flex items-center justify-start w-full h-14 px-72">
      {categoriesData.data.map((category) => {
        return (
          <ul key={category.id}>
            <a href={category.id}>
              <li className="px-2 py-4 hover:text-neutral-200 hover:bg-neutral-600 transition-all ease-in-out duration-200">
                {category.name}
              </li>
            </a>
          </ul>
